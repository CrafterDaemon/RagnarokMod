...
{
    public abstract class RiffInstrumentBase : BigInstrumentItemBase
    {
        ...
        protected void SyncRiffSound(Player player, bool start)
        {
            ...
            if (start)
            {
                if (!SoundEngine.TryGetActiveSound(ragnarokPlayer.fretSlot, out var sound) || !sound.IsPlaying)
                {
                    ...
                    RiffLoader.GetRiff(RiffType)?.OnStart(player);
                }
            }
            else
            {
                ...
                RiffLoader.GetRiff(ragnarokPlayer.activeRiffType)?.OnEnd(player);
