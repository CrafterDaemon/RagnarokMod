    ...
    public class GraniteEnergyStormAI : GlobalNPC
    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (CalamityGamemodeCheck.isBossrush)
            {
                ...
                { // Spawn CoalescedEnergy
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        if (eng < Main.maxNPCs && Main.netMode == NetmodeID.Server)
                        {
                            NetMessage.SendData(MessageID.SyncNPC, -1, -1, null, eng, 0f, 0f, 0f, 0, 0, 0);
                        }
                    }
                ...
                if (npc.ai[3] == 1f)
                {
                    ...
                    for (int j = 0; j < 8; j++)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            if (eng2 < Main.maxNPCs && Main.netMode == NetmodeID.Server)
                            {
                                NetMessage.SendData(MessageID.SyncNPC, -1, -1, null, eng2, 0f, 0f, 0f, 0, 0, 0);
                            }
                        }
                    }
                    npc.netUpdate = true;
                ...
                { // Spawn conduits, now at 75 percent health
                    generate++;
                    if (generate > 240 && Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
                    { // Now generates a new conduit every 4 instead of 10 seconds
                        ...
                        if (minionCount < 10)
                        {  // Now spawns up to 10 conduits instead of 5
                ...
                if (!player.dead && Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
                {
                    npc.ai[0] += 1f;
                    if (npc.ai[0] >= 400f)
                    { // Prepare Charge, prepare time much lower now
                        npc.ai[1] = 0f;
                        npc.velocity.X = 0f;
                        npc.velocity.Y = 0f;
                        for (int k = 0; k < 1; k++)
                        {
                            ...
                            Dust dust = Dust.NewDustDirect(new Vector2(npc.Center.X - 2f, npc.Center.Y - 6f) + offset, 20, 20, DustID.MagicMirror, 0f, 0f, 255, default(Color), 1.5f);
                            dust.noGravity = true;
                            dust.velocity = -offset * 0.075f;
                        }
                    }
                    if (npc.ai[0] >= 430f)
                    {   // Charge-Attack more often
                ...
                if (npc.ai[0] < 400f)
                { // When not charging do regulare movement
                    npc.directionY = ((player.position.Y - 150f > npc.position.Y) ? 1 : -1);
                    if (npc.direction == -1 && npc.velocity.X > -18f)
                    { // 16 instead of 4
            ...
            else if (CalamityGamemodeCheck.isDeath)
            {
                ...
                if (!player.dead && (double)npc.life < (double)npc.lifeMax * 0.75)
                { // Shoot Granite Charges
                    npc.ai[1] += 1f;
                    if (npc.ai[1] >= 180f && Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
                    {
                ...
                {
                    generate++;
                    if (generate > 540 && Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
                    { // Now generates a new conduit every 9 instead of 10 seconds
                        int minionType = ModContent.NPCType<EnergyConduit>();
                        int minionCount = 0;
                        for (int u = 0; u < Main.maxNPCs; u++)
                        {
                        ...
                        if (minionCount < 6)
                        {  // Now spawns up to 6 conduits instead of 5
                            SoundEngine.PlaySound(SoundID.Item70, new Vector2?(npc.Center), null);
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                NPC.NewNPC(source, (int)npc.Center.X, (int)npc.Center.Y, minionType, 0, 0f, 0f, 0f, 0f, 255);
