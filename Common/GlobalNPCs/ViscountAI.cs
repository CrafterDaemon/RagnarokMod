    ...
    public class ViscountAI : GlobalNPC
    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (CalamityGamemodeCheck.isBossrush)
            {
                ...
                { // Ends the fight if there is not valid target{
                    shift = false;
                    openMouth = false;
                    blood = false;
                    stomp = false;
                ...
                { // Select an attack option based on current health
                ...
                { // Ripple attacks
                    ...
                    else if (npc.ai[2] >= 0f && !player.dead && player.HasBuff(ModContent.BuffType<VampiresCurseBuff>()))
                    { // When player is bat
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                ...
                if (npc.ai[1] == 0f)
                {
                    if (npc.ai[0] == 60f)
                    {
                        ...
                        while ((float)num2 < num)
                        {
                            ...
                            int num3 = Dust.NewDust(npc.Center, 0, 0, DustID.GemRuby, 0f, 0f, 100, default(Color), 1.25f);
                    ...
                    if (npc.ai[0] > 60f)
                    { // Viscount Blood attack
                        blood = true;
                        if (!player.dead && npc.ai[0] == (float)(140 - rage) && Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            for (int i = 0; i < 7; i++)
                            {
                        ...
                        if (!player.dead && npc.ai[0] == (float)(190 - rage) && Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            for (int j = 0; j < 12; j++)
                            {
                        ...
                        if (npc.ai[0] == (float)(139 - rage) || npc.ai[0] == (float)(189 - rage) || npc.ai[0] == (float)(249 - rage))
                        {
                            ...
                            while ((float)num5 < num4)
                            {
                                ...
                                int num6 = Dust.NewDust(npc.Center, 0, 0, DustID.Blood, 0f, 0f, 100, default(Color), 1.5f);
                ...
                else if (npc.ai[1] == 2f)
                {
                    ...
                    if (npc.ai[0] == (float)(180 - rage))
                    {
                        ...
                        while ((float)num9 < num8)
                        {
                            ...
                            int num10 = Dust.NewDust(npc.Center, 0, 0, DustID.PureSpray, 0f, 0f, 100, default(Color), 1.5f);
                    ...
                    if (npc.ai[0] == (float)(185 - rage))
                    {
                        ...
                        while ((float)num12 < num11)
                        {
                            ...
                            int num13 = Dust.NewDust(npc.Center, 0, 0, DustID.PureSpray, 0f, 0f, 100, default(Color), 1.5f);
                    ...
                    if (npc.ai[0] >= (float)(190 - rage))
                    {
                        ...
                        while ((float)num15 < num14)
                        {
                            ...
                            int num16 = Dust.NewDust(npc.Center, 0, 0, DustID.PureSpray, 0f, 0f, 100, default(Color), 1.5f);
                ...
                else if (npc.ai[1] == 3f && npc.ai[0] >= 90f)
                {
                    ...
                    {
                        int num18 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.Blood, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 100, default(Color), 1.5f);
                ...
                if ((double)npc.life < (double)npc.lifeMax * 0.33)
                {
                        ...
                        for (int num19 = 0; num19 < 255; num19++)
                        {
                            ...
                            if (player3.active && !player3.dead && player3.DistanceSQ(npc.Center) < 1562500f)
                            {
                                ...
                                while ((float)num21 < num20)
                                {
                                    ...
                                    int num22 = Dust.NewDust(player.Center, 0, 0, DustID.GemRuby, 0f, 0f, 100, default(Color), 1.75f);
                    ...
                    if (npc.ai[3] > 1200f)
                    {
                        ...
                        {
                            int num24 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.GemRuby, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 100, default(Color), 1f);
                        ...
                        while ((float)num26 < num25)
                        {
                            ...
                            int num27 = Dust.NewDust(npc.Center, 0, 0, DustID.GemRuby, 0f, 0f, 100, default(Color), 1.75f);
                        ...
                        for (int num29 = 0; num29 < 255; num29++)
                        {
                            ...
                            if (player4.active && !player4.dead && player4.DistanceSQ(npc.Center) < 1562500f && !player4.HasBuff(BuffID.Stoned) && !player4.HasBuff(num28))
                            {
                                player4.AddBuff(num28, 600, true, false);
                                for (int num30 = 0; num30 < 20; num30++)
                                {
                                    int num31 = Dust.NewDust(player4.position, player4.width, player4.height, DustID.Blood, 0f, 0f, 75, default(Color), 1.25f);
            ...
            else if (CalamityGamemodeCheck.isDeath)
            {
                ...
                { // Select an attack option based on current health
                    if ((double)npc.life < (double)npc.lifeMax * 0.6)
                    { // Summon Bitey Babies as attack option
                        npc.ai[1] = (float)Main.rand.Next(4);
                    }
                    else if ((double)npc.life < (double)npc.lifeMax * 0.75)
                    { // Add Rock Falls to the attack options
                        npc.ai[1] = (float)Main.rand.Next(3);
                    }
                    else if ((double)npc.life < (double)npc.lifeMax * 0.9)
                    { // Add stomp to the attack options
                        npc.ai[1] = (float)Main.rand.Next(2);
                    }
                    else
                    { // Only Ripples and blood attacks
                        npc.ai[1] = 0f;
                    }
                    npc.TargetClosest(true);
                    npc.netUpdate = true;
                ...
                if ((double)npc.life < (double)npc.lifeMax * 0.33)
                {
                    ...
                    if (npc.ai[3] > 1200f)
                    {
                        ...
                        {
                            Player player4 = Main.player[num29];
                            if (player4.active && !player4.dead && player4.DistanceSQ(npc.Center) < 1562500f && !player4.HasBuff(BuffID.Stoned) && !player4.HasBuff(vampirecurse))
                            {
            ...
            { // No bossrush, revengegance or death detected, use original ai instead
                return true;
            }
        }
    }
