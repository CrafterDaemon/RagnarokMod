        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (CalamityGamemodeCheck.isBossrush)
            {
                ...
                {  // Add Beam to moveset
                    ...
                    if (npc.ai[2] == 60f)
                    {
                        ...
                        while ((float)num2 < num)
                        {
                            ...
                            int num3 = Dust.NewDust(player.Center, 0, 0, DustID.ShadowbeamStaff, 0f, 0f, 100, default(Color), 1.5f);
                ...
                {  // Vaporize Blast
                    ...
                    npc.ai[0] += 1f;
                    if (!player.dead && npc.ai[0] >= 30f && npc.ai[0] <= 60f && npc.ai[0] % 10f == 0f && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                ...
                if ((double)npc.life < (double)npc.lifeMax * 0.25)
                {
                    ...
                    if (tripleSpawnTimer >= 180 && !tripleSpawn)
                    {
                        ...
                        if (Main.netMode != NetmodeID.Server)
                        {
                            ...
                            for (int i = 0; i < 25; i++)
                            {
                                Dust.NewDustDirect(npc.Center, 20, 20, DustID.ShadowbeamStaff, Terraria.Utils.NextFloat(Main.rand, -15f, 15f), Terraria.Utils.NextFloat(Main.rand, -15f, 15f), 0, default(Color), 2.5f).noGravity = true;
                            }
