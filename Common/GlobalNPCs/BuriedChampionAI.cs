    ...
    public class BuriedChampionAI : GlobalNPC
    {
        ...
        public override void FindFrame(NPC npc, int frameHeight)
        {
            if ((CalamityGamemodeCheck.isRevengeance || CalamityGamemodeCheck.isBossrush) && OtherModsCompat.shouldRagnarokBossAILoad(ModContent.GetInstance<BossConfig>().champion))
            {
                ...
                if (attackState == 0 && !strike && !charge)
                { // Sword phase
                    framecounter++;
                    if (framecounter > 4)
                    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (CalamityGamemodeCheck.isBossrush)
            {
                ...
                { // Sword phase
                    ...
                    if (npc.ai[2] >= 360f)
                    {
                        ...
                        for (int i = 0; i < 3; i++)
                        {
                            int num = Dust.NewDust(npc.position - npc.velocity, npc.width, npc.height, DustID.Enchanted_Gold, -((float)npc.spriteDirection * 8f), 0f, 100, default(Color), 0.75f);
                    ...
                    if (npc.ai[3] >= 600f)
                    { // Buried dagger every 9 seconds instead of 8
                ...
                if ((double)npc.life < (double)npc.lifeMax * 0.66 && (double)npc.life > (double)npc.lifeMax * 0.33)
                { // Bow phase
                    strike = false;
                    charge = false;
                    attackState = 1;
                    ...
                    if (shifted != 1)
                    {
                        ...
                        for (int j = 0; j < 20; j++)
                        {
                            Dust.NewDust(npc.position, npc.width, npc.height, DustID.Enchanted_Gold, (float)Main.rand.Next(-6, 6), (float)Main.rand.Next(-6, 6), 255, default(Color), 1.5f);
                        }
                        shifted = 1;
                    }
                    ...
                    if (!player.dead && npc.ai[2] >= 540f)
                    { // Special arrow attack every 5 instead of 10 seconds
                        SoundEngine.PlaySound(SoundID.Item102, new Vector2?(npc.Center), null);
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                    ...
                    if (!player.dead && npc.ai[3] >= 300f && Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
                    {
                        if (npc.ai[3] == 300f)
                        {
                            ...
                            for (int k = 0; k < 10; k++)
                            {
                                Dust.NewDust(npc.position, npc.width, npc.height, DustID.Enchanted_Gold, (float)Main.rand.Next(-6, 6), (float)Main.rand.Next(-6, 6), 0, default(Color), 1.25f);
                            }
                        }
                        npc.velocity.X = 0f;
                        npc.velocity.Y = 0f;
                ...
                if ((double)npc.life <= (double)npc.lifeMax * 0.33)
                {
                    ...
                    if (shifted != 2)
                    {
                        ...
                        for (int m = 0; m < 20; m++)
                        {
                            Dust.NewDust(npc.position, npc.width, npc.height, DustID.MagicMirror, (float)Main.rand.Next(-6, 6), (float)Main.rand.Next(-6, 6), 255, default(Color), 1.5f);
                        }
                    ...
                    if (npc.ai[2] > 300f)
                    {
                        ...
                        for (int n = 0; n < 1; n++)
                        {
                            int num6 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.MushroomSpray, 0f, 0f, 100, default(Color), 1.25f);
                    ...
                    if (npc.ai[2] == 360f || npc.ai[2] == 420f || npc.ai[2] == 480f)
                    {
                        ...
                        {
                            int num10 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.MushroomSpray, (float)Main.rand.Next(-3, 3), (float)Main.rand.Next(-3, 3), 0, default(Color), 1.25f);
                        ...
                        if (num11 < Main.maxNPCs && Main.netMode == NetmodeID.Server)
                        {
                            NetMessage.SendData(MessageID.SyncNPC, -1, -1, null, num11, 0f, 0f, 0f, 0, 0, 0);
                        }
                        if (npc.ai[2] == 480f)
                        {
                            npc.ai[2] = -300f;
            ...
            else if (CalamityGamemodeCheck.isDeath)
            {
                ...
                { // Bow phase
                    ...
                    if (!player.dead && npc.ai[2] >= 540f)
                    { // Special arrow attack every 9 instead of 10 seconds
                        SoundEngine.PlaySound(SoundID.Item102, new Vector2?(npc.Center), null);
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Projectile.NewProjectile(source_FromAI, npc.Center.X, npc.Center.Y, 3.5f, 0f, ModContent.ProjectileType<BuriedArrow2>(), 20, 0f, Main.myPlayer, 0f, 0f, 0f);
            ...
            else if (CalamityGamemodeCheck.isRevengeance)
            {
                ...
                { // Bow phase
                    ...
                    if (!player.dead && npc.ai[3] >= 300f && Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
                    {
                        ...
                        npc.velocity.Y = 0f;
                        npc.ai[0] = 0f;
                        if (npc.ai[3] == 360f || npc.ai[3] == 390f || npc.ai[3] >= 420f)
                        { // Three shots
                            int arrowtype = 113;
                            float num3 = 1f;
                            int num4 = ModContent.ProjectileType<BuriedArrowC>();
