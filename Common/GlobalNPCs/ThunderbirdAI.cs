        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (CalamityGamemodeCheck.isBossrush)
            {
                ...
                else if ((getAnimationState(npc) == 0 && getAttackState(npc) != 1) || (getAttackState(npc) == 1 && getAttackStateTimer(npc) < 60f))
                {
                    ...
                    {
                        NPC npc2 = npc;
                        npc2.velocity.Y = npc2.velocity.Y - ((npc.velocity.Y > 0f) ? 1.2f : 0.13f);
                    }
                    if (player.position.Y > npc.position.Y + (float)desiredaltitudeaboveplayer)
                    {
                        NPC npc3 = npc;
                    ...
                    if (player.Center.X < npc.position.X && npc.velocity.X > -11f)
                    {
                    ...
                    else if (player.Center.X > npc.position.X && npc.velocity.X < 11f)
                    {
                ...
                if (getChargeTimer(npc) < 5f)
                {
                    ...
                    {
                        int dustparam = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width, npc.height, DustID.MagicMirror, npc.velocity.X * 0.1f, -5f, 125, default(Color), 1f);
                    ...
                    else if (getAttackState(npc) == 1) // Charge
                    {
                        ...
                        if (getAttackStateTimer(npc) > 120f && getAttackStateTimer(npc) < 240f)
                        {
                        ...
                        if ((getAttackStateTimer(npc) == 240f || getAttackStateTimer(npc) == 410f) && Main.netMode != NetmodeID.Server)
                        {
                            for (int k = 0; k < 20; k++)
                            {
                                int chargedustparam = Dust.NewDust(npc.position, npc.width, npc.height, DustID.Enchanted_Gold, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 0, default(Color), 3f);
                                Main.dust[chargedustparam].noGravity = true;
                            }
                            for (int l = 0; l < 20; l++)
                            {
                                int chargedustparam = Dust.NewDust(npc.position, npc.width, npc.height, DustID.GemSapphire, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 0, default(Color), 3f);
                                Main.dust[chargedustparam].noGravity = true;
                            }
                        }
                        if (getAttackStateTimer(npc) > 240f && getAttackStateTimer(npc) < 290f)
                        {
                            int chargedustparam = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.MagicMirror, 0f, 0f, 255, default(Color), 1.2f);
                        ...
                        else if ((getAttackStateTimer(npc) >= 290f && getAttackStateTimer(npc) < 410f) || (getAttackStateTimer(npc) >= 460f && getAttackStateTimer(npc) < 580f))
                        {
                            int chargedirection = ((chargeDecide == 1) ? (-1) : 1);
                            npc.direction = chargedirection;
                            npc.spriteDirection = chargedirection;
                        ...
                        else if ((getAttackStateTimer(npc) >= 410f && getAttackStateTimer(npc) < 460) || getAttackStateTimer(npc) >= 580f)
                        {
                            int chargedustparam = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.MagicMirror, 0f, 0f, 255, default(Color), 1.2f);
                            Dust dust2 = Main.dust[chargedustparam];
                            dust2.velocity *= 0.2f;
                        ...
                        if (getAttackStateTimer(npc) > 630f)
                        {
                            if (Main.netMode != NetmodeID.Server)
                            {
                                for (int m = 0; m < 20; m++)
                                {
                                    int num12 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.Enchanted_Gold, (float)Main.rand.Next(-5, 5), (float)Main.rand.Next(-5, 5), 0, default(Color), 3f);
                                    Main.dust[num12].noGravity = true;
                                }
                                for (int n = 0; n < 20; n++)
                                {
                                    int num13 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.GemSapphire, (float)Main.rand.Next(-5, 5), (float)Main.rand.Next(-5, 5), 0, default(Color), 3f);
                    ...
                    else if (getAttackState(npc) == 2)
                    {
                        ...
                        if (getAttackStateTimer(npc) >= 60f && getAttackStateTimer(npc) % 60f == 0f && Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            if (getAttackStateTimer(npc) % 120f == 0)
                            {
                    ...
                    else if (getAttackState(npc) == 3)
                    {
                        float attacktimer = getAttackStateTimer(npc);
                        setAttackStateTimer(npc, attacktimer + 1f);
                        if (getAttackStateTimer(npc) == 10f && Main.netMode != NetmodeID.Server)
                        {
                            for (int num15 = 0; num15 < 15; num15++)
                            {
                                int num16 = Dust.NewDust(npc.Center, 30, 30, DustID.Enchanted_Gold, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 0, default(Color), 2f);
                                Main.dust[num16].noGravity = true;
                            }
                        }
                        if (getAttackStateTimer(npc) >= 30f && getAttackStateTimer(npc) % 10f == 0f)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                ...
                else
                {
                    ...
                    if (getChargeTimer(npc) == 6f)
                    {
                        ...
                        {
                            int num18 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.MagicMirror, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 255, default(Color), 3f);
                    ...
                    if (!player.dead && getChargeTimer(npc) % 30f == 0f && Main.netMode != NetmodeID.MultiplayerClient)
                    {
            ...
            else if (CalamityGamemodeCheck.isDeath)
            {
                ...
                else if ((getAnimationState(npc) == 0 && getAttackState(npc) != 1) || (getAttackState(npc) == 1 && getAttackStateTimer(npc) < 60f))
                {
                    ...
                    {
                        NPC npc2 = npc;
                        npc2.velocity.Y = npc2.velocity.Y - ((npc.velocity.Y > 0f) ? 1.0f : 0.1f);
                    }
                    if (player.position.Y > npc.position.Y + (float)desiredaltitudeaboveplayer)
                    {
                        NPC npc3 = npc;
                    ...
                    npc.rotation = npc.velocity.X * 0.05f;
                    if (player.Center.X < npc.position.X && npc.velocity.X > -8f)
                    {
                    ...
                    else if (player.Center.X > npc.position.X && npc.velocity.X < 7f)
                    {
                        npc.velocity.X = npc.velocity.X + 0.11f;
                        if (npc.velocity.X < -8f)
                        {
                ...
                if (getChargeTimer(npc) < 5f)
                {
                    ...
                    else if (getAttackState(npc) == 1)
                    {
                        ...
                        if (getAttackStateTimer(npc) == 350f)
                        {
                            chargeDecide = (byte)((player.Center.X < npc.Center.X) ? 1 : 2);
                            npc.netUpdate = true;
                        }
                        ...
                        if ((getAttackStateTimer(npc) == 300f || getAttackStateTimer(npc) == 470f) && Main.netMode != NetmodeID.Server)
                        {
                            for (int k = 0; k < 20; k++)
                            {
                                int chargedustparam = Dust.NewDust(npc.position, npc.width, npc.height, DustID.Enchanted_Gold, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 0, default(Color), 3f);
                        ...
                        if (getAttackStateTimer(npc) > 300f && getAttackStateTimer(npc) < 350f)
                        {
                            int chargedustparam = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.MagicMirror, 0f, 0f, 255, default(Color), 1.2f);
                            Dust dust2 = Main.dust[chargedustparam];
                            dust2.velocity *= 0.2f;
                        ...
                        else if (getAttackStateTimer(npc) >= 350f && getAttackStateTimer(npc) < 470f)
                        {
                            int chargedirection = ((chargeDecide == 1) ? (-1) : 1);
                            npc.direction = chargedirection;
                            npc.spriteDirection = chargedirection;
                        ...
                        else if (getAttackStateTimer(npc) >= 470f)
                        {
                            int chargedustparam = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.MagicMirror, 0f, 0f, 255, default(Color), 1.2f);
                            Dust dust2 = Main.dust[chargedustparam];
                            dust2.velocity *= 0.2f;
            ...
            else if (CalamityGamemodeCheck.isRevengeance)
            {
                ...
                if (getAttackState(npc) != 1)
                {
                    if (player.Center.X + 850f < npc.Center.X)
                    {
                        npc.velocity.X = ((npc.velocity.X < 0f) ? (-7f) : (-5f));
                    }
                    if (player.Center.X - 850f > npc.Center.X)
                    {
                        npc.velocity.X = ((npc.velocity.X < 0f) ? 7f : 5f);
                ...
                else if ((getAnimationState(npc) == 0 && getAttackState(npc) != 1) || (getAttackState(npc) == 1 && getAttackStateTimer(npc) < 60f))
                {
                    ...
                    {
                        NPC npc2 = npc;
                        npc2.velocity.Y = npc2.velocity.Y - ((npc.velocity.Y > 0f) ? 0.9f : 0.08f);
                    }
                    if (player.position.Y > npc.position.Y + (float)desiredaltitudeaboveplayer)
                    {
                        NPC npc3 = npc;
                    ...
                    npc.rotation = npc.velocity.X * 0.05f;
                    if (player.Center.X < npc.position.X && npc.velocity.X > -5f)
                    {
                ...
                if (getChargeTimer(npc) < 5f)
                {
                    ...
                    else if (getAttackState(npc) == 1)
                    {
                        ...
                        if (getAttackStateTimer(npc) == 300f && Main.netMode != NetmodeID.Server)
                        {
                            for (int k = 0; k < 20; k++)
                            {
                                int num8 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.Enchanted_Gold, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 0, default(Color), 3f);
                                Main.dust[num8].noGravity = true;
                            }
                            for (int l = 0; l < 20; l++)
                            {
                                int num9 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.GemSapphire, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 0, default(Color), 3f);
                                Main.dust[num9].noGravity = true;
                            }
                        }
                        if (getAttackStateTimer(npc) > 300f)
                        {
                            int num10 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.MagicMirror, 0f, 0f, 255, default(Color), 1.2f);
